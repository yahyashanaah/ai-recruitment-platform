  ...
  return (
    <div className={cn("flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between", className)}>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        {eyebrow && <p className="text-xs uppercase tracking-[0.32em] text-white/35">{eyebrow}</p>}
        <h2 className="font-display mt-2 text-3xl font-semibold tracking-tight text-white md:text-4xl">
          {title}
        </h2>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-white/56 md:text-base">{description}</p>
      </motion.div>
